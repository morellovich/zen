...
- 🧼 Enforced linting, module boundaries, and formatting rules. See the [Linting & Architecture Conventions](docs/linting-and-conventions.md) guide.
...
We are utilizing pnpm as our package manager.
...
# Install node modules & approve build scripts (for pnpm 10+)
pnpm i
pnpm approve-builds --all

# Start the PostgreSQL server (listens on host port 5433 to avoid standard port 5432 conflicts)
docker compose up -d
...
```bash
# 1. Start the Nest API (GraphQL server at http://localhost:7080/graphql)
pnpm start:api

# 2. (Optional) Re-generate frontend GraphQL types once API is running
pnpm gen:frontend

# 3. Start the Angular app at http://localhost:4200
pnpm start
```

> **Note for CI / Sandboxed / Container Environments**:
> If Nx encounters socket timeouts or plugin worker connection failures, run commands with:
> `NX_ISOLATE_PLUGINS=false NX_DAEMON=false pnpm start:api` and `NX_ISOLATE_PLUGINS=false NX_DAEMON=false pnpm start`.
...
- [x] Custom pnpm scripts to code generate the Nest GraphQL api on Prisma schema changes.
...
- [x] [Electron](https://www.electronjs.org/) branch with new pnpm script `start:electron` to launch the app within Electron.
...
- [x] pnpm scripts for versioned releases of the Nest **api** server as a containerized Docker image. [node:24-alpine](https://hub.docker.com/_/node?tab=description&ref=hackernoon.com) is used as the Docker container base image and the appropriate `tsconfig.app.json` configurations are being applied for the Nest app.
