...
- Build the API docker image via `pnpm prod:api`.  It will utilize the production environment variables at `apps/api/src/environments/environment.prod.ts` and produce a Docker image tagged `zen-api:latest`
...
- Run `pnpm start` to start the Angular app to test the API running in k8s

- To build the static HTML/CSS assets for production, utilize the script `pnpm prod:portal`